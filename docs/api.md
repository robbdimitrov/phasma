...
- `type=all`: a single blended, ranked page mixing all three entity types.
  Not currently called by the frontend (typeahead makes separate per-type
  requests; its results page is posts-only — see `docs/frontend.md`); kept
  as a general-purpose blended mode. Roughly a 20/60/20 users/posts/hashtags
  split per page (`computeBlendTargets`, min 1 user/1 hashtag once
  `limit >= 3`). Items are `{"type": "users"|"posts"|"hashtags", "item": <the
  type's normal item shape>}`. Users the viewer follows are boosted to the
  front of the page's user results (never across pages). Cursor encodes
  independent per-index offsets (opaque to the client). A page can return
