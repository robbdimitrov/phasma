...
└── (app)/                  +layout.server.ts: GET /users/me → currentUser: User | null, no redirect; unread count only when authenticated
    ...
    └── (private)/          +layout.server.ts: redirect /login if currentUser absent
        ...
        ├── search/             load: GET /search?q=&type=posts (small preview limit) → results are always a posts-only image grid; `@name` prefix is stripped before searching (users are handled by the typeahead, not by results), `#tag` prefix stays intact for the backend's exact-hashtag filter; clearing the query navigates back to /search, reverting to discovery; empty query shows discovery content (suggested users via GET /users/suggested + popular posts grid via GET /posts/popular) below the input; the input's dropdown is one of two mutually-exclusive modes, both absolutely positioned so they overlap the discovery/results content below: focusing the input while it's empty shows recent searches (GET /search/recent, capped at 10) via RecentSearches.svelte, replaced by live suggestions (GET /suggest, users/hashtags only, no posts, alternated client-side via `interleaveSuggestions()`) once a query is typed; recording a recent search: a typeahead suggestion click (mouse or keyboard) POSTs `{type: "users"|"hashtags", reference}`, a form submission POSTs `{type: "posts", reference: <raw value>}` (fire-and-forget, via /search/recent +server.ts), both from `lib/utils/recentSearch.ts`; removeRecent/clearRecent form actions optimistically update the dropdown's lifted `recentItems` state without a full reload, reverting only on failure; while either dropdown is open, a `bg-base-100/70 backdrop-blur-sm` scrim dims the discovery/results content below (sized via `inset-0` to that content's own wrapper, not a fixed height, so it can't inflate page scroll height) so the dropdown reads as a focused overlay rather than clipping over unrelated content
