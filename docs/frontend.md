...
└── (app)/                  +layout.server.ts: GET /users/me only when a session cookie exists → currentUser: User | null, no redirect; unread count only when authenticated
    ...
    └── (private)/          +layout.server.ts: redirect /login if currentUser absent
        ...
        ├── notifications/      load: GET /notifications (read-only); mark-all-unread-read is a client POST fired on mount/load-more, never from load
        ├── search/             load: GET /search?q=&type=posts (small preview limit) → results are always a posts-only image grid; `@name` prefix is stripped before searching (users are handled by the typeahead, not by results), `#tag` prefix stays intact for the backend's exact-hashtag filter; clearing the query navigates back to /search, reverting to discovery; empty query shows discovery content (suggested users via GET /users/suggested + popular posts grid via GET /posts/popular); the input's dropdown has two mutually-exclusive modes, both absolutely positioned over the discovery/results content below: focusing the input while empty shows recent searches (GET /search/recent, capped at 10) via RecentSearches.svelte, replaced by live suggestions (GET /suggest, users/hashtags only, no posts, alternated client-side via `interleaveSuggestions()`) once a query is typed; recording a recent search: a typeahead suggestion click (mouse or keyboard) POSTs `{type: "users"|"hashtags", reference}`, a form submission POSTs `{type: "posts", reference: <raw value>}` (fire-and-forget, via /search/recent +server.ts), both from `lib/utils/recentSearch.ts`; removeRecent/clearRecent form actions optimistically update the dropdown's lifted `recentItems` state without a full reload, reverting only on failure; while either dropdown is open, a `bg-base-100/70 backdrop-blur-sm` scrim dims the discovery/results content below (sized via `inset-0` to that content's own wrapper, not a fixed height, so it can't inflate page scroll height), reading as a focused overlay rather than clipping over unrelated content
        ├── search/recent/      POST +server.ts — proxy: POST /search/recent (fire-and-forget recording, called directly by client components, not on load)
...
| `/notifications`             | POST   | +server.ts        | PUT /notifications/{id}/read for the given ids, fired client-side on mount/load-more                        |
| `/search`                    | GET    | page load         | GET /search?type=posts (limit=5); @ prefix stripped, # prefix kept for hashtag filter; empty query also loads GET /search/recent |
| `/search`                    | GET    | +server.ts        | GET /search?type=posts&cursor= ("Load more" continuation)                                                   |
| `/search?/removeRecent`      | POST   | named form action | DELETE /search/recent/{id}                                                                                   |
| `/search?/clearRecent`       | POST   | named form action | DELETE /search/recent                                                                                        |
| `/search/recent`             | POST   | +server.ts        | POST /search/recent (fire-and-forget, called by client components on suggestion click / form submit)        |
...
`/settings/sessions`'s server load calls `GET /sessions` through `apiClient`,
maps the API timestamps to `Date` values, and renders the initial list during
SSR; the browser does not fetch sessions on mount.
